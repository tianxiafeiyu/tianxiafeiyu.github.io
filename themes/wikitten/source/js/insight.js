...
;(function ($, CONFIG) {
        ...
        switch (type) {
            ...
            case "PAGES":
                $searchItems = array.map(function (item) {
                    ...
                    if (!title) {
                        title = item.path.substring(0, item.path.length - 1)
                    ...
                    return searchItem("rocket", title, null, preview, CONFIG.ROOT_URL + item.path)
                })
                break
            case "CATEGORIES":
            case "TAGS":
    ...
    function filterFactory(keywords) {
        return {
            POST: function (obj) {
                return filter(keywords, obj, ["title", "text", "path"])
            },
            PAGE: function (obj) {
                return filter(keywords, obj, ["title", "text", "path"])
            },
    ...
    $(".ins-search-mask").click(() => {
        console.log("mask click")
        $main.removeClass("show")
    })

    $main.find(".ins-search-input").blur(() => {
